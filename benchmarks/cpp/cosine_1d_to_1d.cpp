...
float cosineSimilarity(const vector<float>& v1, const vector<float>& v2, int mantissaBits, MulFunc mulFunc) {
    float dotProduct = 0;
    float norm1 = 0;
    float norm2 = 0;
...
float standardCosineSimilarity(const vector<float>& v1, const vector<float>& v2) {
    float dotProduct = 0;
    float norm1 = 0;
    float norm2 = 0;
...
int main() {
    ...
    vector<float> v2 = {4.085858, 5.0, 6.0};
    int N = 1000000;

    long long standardTotalTime = 0;
    float standardSimilarity = 0;

    for (int i = 0; i < N; i++) {
        auto start = high_resolution_clock::now();
        standardSimilarity = standardCosineSimilarity(v1, v2);
    ...
    cout << "Standard Cosine Similarity: " << standardSimilarity << endl;
    ...
    for (int mantissaBits = 2; mantissaBits <= 23; mantissaBits++) {
        ...
        for (int i = 0; i < N; i++) {
            auto start = high_resolution_clock::now();
            float similarityLMul = cosineSimilarity(v1, v2, mantissaBits, lMul);
            auto stop = high_resolution_clock::now();
            auto durationLMul = duration_cast<nanoseconds>(stop - start);

            start = high_resolution_clock::now();
            float similarityMul = cosineSimilarity(v1, v2, mantissaBits, mul);
            stop = high_resolution_clock::now();
            auto durationMul = duration_cast<nanoseconds>(stop - start);

            lMulMSE += pow(similarityLMul - standardSimilarity, 2);
            mulMSE += pow(similarityMul - standardSimilarity, 2);
